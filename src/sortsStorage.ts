...
type SortParamsPartial = Partial<import('./sortTypes').SortParams> & {
...
export function loadSortsCollection(): SortsCollection {
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<SortsCollection>
    ...
    const activeSortId = sorts.some((s) => s.id === parsed.activeSortId)
      ? String(parsed.activeSortId)
      : sorts[0].id
    const farm = { ...DEFAULT_FARM, ...(parsed.farm ?? {}) }
...
export function initCollectionFromCalculator(state: CalculatorState): SortsCollection {
  const collection = defaultCollection()
  collection.farm = extractFarmSettings(state)
  collection.sorts[0] = {
    ...collection.sorts[0],
    params: extractSortParams(state),
  }
  return collection
}
...
export function persistFromCalculator(
  ...
  const sorts = collection.sorts.map((s) => (s.id === activeSortId ? { ...s, params } : s))
  const next = { ...collection, farm, sorts, activeSortId }
  saveSortsCollection(next)
  return next
}

export function addSort(collection: SortsCollection): SortsCollection | null {
  if (collection.sorts.length >= MAX_SORTS) return null
  const index = collection.sorts.length + 1
  const active = collection.sorts.find((s) => s.id === collection.activeSortId)
  const sorts = [...collection.sorts, createSortProfile(index, active?.params ?? DEFAULT_SORT_PARAMS)]
...
export function removeSort(collection: SortsCollection, sortId: string): SortsCollection | null {
  ...
  const activeSortId =
    collection.activeSortId === sortId ? sorts[0].id : collection.activeSortId
...
export function initAppSortsState(urlState: CalculatorState): {
  ...
  let collection = loadSortsCollection()
